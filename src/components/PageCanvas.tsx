...
export function PageCanvas({ doc, page, pageIndex, selectedBlockId, onSelectBlock, onUpdatePage, onOpenDocumentSettings, readonly = false }: {
  ...
  const action = (id: string, a: BlockAction) => {
    ...
    if (a === 'duplicate') {
      const next = clone(page.blocks[index]); next.id = `${next.id}-copy-${Date.now().toString(36)}`
      const blocks = [...page.blocks]; blocks.splice(index + 1, 0, next); onUpdatePage({ ...page, blocks }); return
    }
    const to = a === 'up' ? index - 1 : index + 1
    if (to < 0 || to >= page.blocks.length) return
    const blocks = [...page.blocks]; [blocks[index], blocks[to]] = [blocks[to], blocks[index]]; onUpdatePage({ ...page, blocks })
  }

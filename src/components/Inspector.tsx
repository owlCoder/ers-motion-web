...
export function Inspector({ doc, block, onDocumentChange, onBlockChange, onClose }: {
  ...
  if (!block) {
    return (
      <aside className="inspector no-print">
        <div className="panel-title"><div><b>Dokument</b><span>Podešavanja izgleda i metapodataka</span></div>{onClose && <button onClick={onClose}><Icon name="close" size={16} /></button>}</div>
        ...
        <div className="panel-section">
          <div className="section-label">Tipografija i stil</div>
        ...
        <div className="panel-section">
          ...
          <Field label="Zaglavlje"><TextInput value={doc.headerText || ''} onChange={(e) => patchDoc({ headerText: e.target.value })} /></Field>
          <Field label="Podnožje"><TextInput value={doc.footerText || ''} onChange={(e) => patchDoc({ footerText: e.target.value })} /></Field>
        </div>
      </aside>
    )
  }
  ...
  return (
    <aside className="inspector no-print">
      <div className="panel-title"><div><b>Izabrani blok</b><span>{block.type}</span></div>{onClose && <button onClick={onClose}><Icon name="close" size={16} /></button>}</div>
      ...
      {block.type === 'text' && <>
        ...
        <div className="panel-section">
          <div className="section-label">Formatiranje</div>
          ...
          <small className="muted">Označite tekst na stranici i primenite format.</small>
        </div>
      </>}
      ...
      {block.type === 'code' && <div className="panel-section">
        ...
        <Field label="Opis"><TextInput value={block.caption || ''} onChange={(e) => patch({ ...block, caption: e.target.value })} /></Field>
      ...
      {block.type === 'diagram' && <div className="panel-section">
        ...
        {block.items.map((item, i) => <div className="diagram-item-editor" key={item.id}>
          ...
          <TextInput value={item.subtitle || ''} placeholder="Kratak opis" onChange={(e) => { const items = [...block.items]; items[i] = { ...item, subtitle: e.target.value }; patch({ ...block, items }) }} />
        ...
        <button className="secondary full" onClick={() => patch({ ...block, items: [...block.items, { id: uid('item'), title: 'Novi element', subtitle: 'opis', accent: 'blue' }] })}><Icon name="plus" size={14} /> Dodaj element</button>
        <Field label="Opis ispod dijagrama"><TextInput value={block.footer || ''} onChange={(e) => patch({ ...block, footer: e.target.value })} /></Field>
      </div>}
      ...
      {block.type === 'image' && <div className="panel-section">
        <Field label="Slika"><input type="file" accept="image/*" className="file-input" onChange={(e) => { const file = e.target.files?.[0]; if (file) readImage(file, (src) => patch({ ...block, src })) }} /></Field>
        <Field label="Opis"><TextInput value={block.caption || ''} onChange={(e) => patch({ ...block, caption: e.target.value })} /></Field>
      ...
      {block.type === 'institution' && <div className="panel-section">
        <Field label="Univerzitet"><TextInput value={stripHtml(block.university)} onChange={(e) => patch({ ...block, university: e.target.value })} /></Field>
        <Field label="Fakultet"><TextInput value={stripHtml(block.faculty)} onChange={(e) => patch({ ...block, faculty: e.target.value })} /></Field>
        <Field label="Departman / odsek"><TextInput value={stripHtml(block.department || '')} onChange={(e) => patch({ ...block, department: e.target.value })} /></Field>
        <Field label="Levi logo"><input type="file" accept="image/*" className="file-input" onChange={(e) => { const file = e.target.files?.[0]; if (file) readImage(file, (leftLogoSrc) => patch({ ...block, leftLogoSrc })) }} /></Field>
        <Field label="Desni logo"><input type="file" accept="image/*" className="file-input" onChange={(e) => { const file = e.target.files?.[0]; if (file) readImage(file, (rightLogoSrc) => patch({ ...block, rightLogoSrc })) }} /></Field>
      </div>}

      {block.type === 'divider' && <div className="panel-section"><span className="muted">Razdelna linija između celina sadržaja.</span></div>}
    </aside>
  )
}

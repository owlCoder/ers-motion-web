...
const useStyles = makeStyles({
  ...
  headerTop: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 12px 6px' },
  titleStack: { display: 'flex', flexDirection: 'column', gap: '1px' },
  tabs: { paddingLeft: '8px', minHeight: '34px' },
  section: { display: 'flex', flexDirection: 'column', gap: '10px', padding: '13px 12px' },
  sectionTitle: { textTransform: 'uppercase', letterSpacing: '.07em', color: '#777', fontWeight: 600 },
  twoCol: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' },
  accentRow: { display: 'flex', flexWrap: 'wrap', gap: '8px' },
    ...
    <aside className={styles.root}>
      <div className={styles.header}>
        <div className={styles.headerTop}>
          <span className={styles.titleStack}><Text weight="semibold">Inspector</Text><Caption1>{block ? `Selected: ${block.type}` : 'Document properties'}</Caption1></span>
        ...
        <TabList className={styles.tabs} selectedValue={block ? 'selection' : 'document'}>
          <Tab value="document" onClick={() => {}}>Document</Tab>
          <Tab value="selection" disabled={!block}>Selection</Tab>
        </TabList>
      </div>
      ...
      {!block && <>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.twoCol}>
            <Field label="Typography"><Select value={doc.theme.font} onChange={(event) => patchTheme({ font: event.target.value as DocumentTheme['font'] })}><option value="System">Segoe UI</option><option value="Serif">Georgia</option><option value="Humanist">Trebuchet MS</option></Select></Field>
            <Field label="Density"><Select value={doc.theme.density} onChange={(event) => patchTheme({ density: event.target.value as DocumentTheme['density'] })}><option value="comfortable">Comfortable</option><option value="compact">Compact</option></Select></Field>
          </div>
          <Field label="Accent color">
            <div className={styles.accentRow}>{(Object.keys(ACCENTS) as Accent[]).map((accent) => <Tooltip key={accent} content={accent} relationship="label"><Button appearance="outline" className={mergeClasses(styles.accentButton, doc.theme.accent === accent && styles.accentSelected)} style={{ backgroundColor: ACCENTS[accent].solid }} onClick={() => patchTheme({ accent })} aria-label={accent} /></Tooltip>)}</div>
          </Field>
        </div>
        <Divider />
        <div className={styles.section}>
      ...
      {block?.type === 'text' && <>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.formatRow}>
            <Tooltip content="Bold" relationship="label"><Button appearance="subtle" icon={<Icon name="bold" size={16} />} onMouseDown={(event) => { event.preventDefault(); document.execCommand('bold') }} /></Tooltip>
            <Tooltip content="Italic" relationship="label"><Button appearance="subtle" icon={<Icon name="italic" size={16} />} onMouseDown={(event) => { event.preventDefault(); document.execCommand('italic') }} /></Tooltip>
            <Tooltip content="Underline" relationship="label"><Button appearance="subtle" icon={<Icon name="underline" size={16} />} onMouseDown={(event) => { event.preventDefault(); document.execCommand('underline') }} /></Tooltip>
          </div>
          <Caption1 className={styles.hint}>Select text on the page, then apply formatting.</Caption1>
        </div>
      </>}
      ...
      {block?.type === 'callout' && <div className={styles.section}>
        <Caption1 className={styles.sectionTitle}>Callout</Caption1>
        <Field label="Type"><Select value={block.tone} onChange={(event) => patch({ ...block, tone: event.target.value as CalloutTone })}><option value="info">Information</option><option value="note">Note</option><option value="task">Task</option><option value="warning">Warning</option><option value="success">Result</option></Select></Field>
      </div>}
      ...
      {block?.type === 'diagram' && <div className={styles.section}>
        ...
        <div className={styles.repeatStack}>{block.items.map((item, index) => <div className={styles.diagramEditor} key={item.id}>
          <div className={styles.repeatRow}><Input value={item.title} onChange={(_, data) => { const items = [...block.items]; items[index] = { ...item, title: data.value }; patch({ ...block, items }) }} /><Button appearance="subtle" className={styles.destructive} icon={<Icon name="trash" size={14} />} onClick={() => patch({ ...block, items: block.items.filter((candidate) => candidate.id !== item.id) })} /></div>
          <Input value={item.subtitle || ''} placeholder="Short description" onChange={(_, data) => { const items = [...block.items]; items[index] = { ...item, subtitle: data.value }; patch({ ...block, items }) }} />
          <Select value={item.accent || 'blue'} onChange={(event) => { const items = [...block.items]; items[index] = { ...item, accent: event.target.value as Accent }; patch({ ...block, items }) }}>{Object.keys(ACCENTS).map((accent) => <option key={accent} value={accent}>{accent}</option>)}</Select>
        </div>)}</div>
        ...
        <Field label="Diagram caption"><Input value={block.footer || ''} onChange={(_, data) => patch({ ...block, footer: data.value })} /></Field>
      </div>}
      ...
      {block?.type === 'divider' && <div className={styles.section}><Caption1 className={styles.hint}>A simple divider between document sections.</Caption1></div>}
    </aside>
  )
}
